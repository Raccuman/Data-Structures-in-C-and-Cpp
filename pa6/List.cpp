...
* List.cpp
*  this is where the List classes and functions get filled out
***/
...
ListElement List::peekPrev() const{ // needs more work
   if( !(position()> 0)){
      throw std::length_error("List: peekPrev(): position is zero so you cant look backwards, there is nothing there");
   }
...
void List::cleanup() {
    ...
    if (temp_curs > length()){ // cursor re-setting
        temp_curs = length();
    }
    pos_cursor = temp_curs;
}
...
void List::reverse() {
    ...
    // After the loop, `temp` is at the last node, update the sentinels
    temp = frontDummy;
    frontDummy = backDummy;
    backDummy = temp;

    // Update dummy node connections
